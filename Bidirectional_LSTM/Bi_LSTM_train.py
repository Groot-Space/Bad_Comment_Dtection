...
os.chdir("..")
train_data = W2V.read_data("./Word2Vec/Movie_rating_data/ratings_train.txt") #워드투백 모델 데이터
test_data = W2V.read_data("./Word2Vec/Movie_rating_data/ratings_test.txt") #워드 투 백 모델 테스트 데이터
...
train_X = train_tokens[:,0] #문자열
train_Y = train_tokens[:,1] #결과값
...
Vector_size = 300
train_seq_length = [len(x) for x in train_X] #모든 문자열의 길이
test_seq_length = [len(x) for x in test_X]
Maxseq_length = max(train_seq_length) ## 95
learning_rate = 0.001
lstm_units = 128 #히든레이어 노드 갯수
num_class = 2
training_epochs = 4

#인풋과 타켓데이터를 입력받기 위한 플레이홀더 생성
...
#마지막 히든레이어의 출력을 소프트맥스 출력값으로 변환해주기 위한 변수 선언
